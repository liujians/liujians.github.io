...
var S = {
  init: function () {
    ...
    S.ShapeBuilder.init();
    S.UI.init();
    document.querySelector('.mycanvas').classList.add('body--ready');

    if (i !== -1) {
      S.UI.simulate(decodeURI(action).substring(i + 3));
    } else {
      S.UI.simulate('The time is|#time|#countdown 3|welcome|to|liujians|blog|#icon heart|let\'s again?');
    }
...
S.UI = (function () {
  ...
  function performAction(value) {
    ...
    timedAction(function () {
      ...
      case 'icon':
        S.ShapeBuilder.imageFile('/static/plugin-shape/font-awesome/' + value + '.png', function (obj) {
...
S.ShapeBuilder = (function () {
  ...
  function fit() {
    shapeCanvas.width = Math.floor(CANVASWIDTH / gap) * gap;
    shapeCanvas.height = Math.floor(CANVASHEIGHT / gap) * gap;
